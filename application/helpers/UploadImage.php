...
class UploadImage
{
    ...
    private $messages = [
        'upload_no_file_types'        => 'File type is not allowed',
        'upload_no_filepath'          => 'File path is not provided',
        'upload_not_writable'         => 'Upload directory is not writable',
        'upload_bad_filename'         => 'Filename is not appropriate',
        'upload_destination_error'    => 'Upload destination does not match',
        'upload_unable_to_write_file' => 'File content is not valid',
        'upload_invalid_dimensions'   => 'File dimensions is not valid',
        'upload_invalid_filesize'     => 'File size is not valid',
        'upload_invalid_filetype'     => 'File type is not valid'
    ];
     ...
     public static function getMimes()
    {
            ...
            if (empty($_mimes))
            {
                if (file_exists($_SERVER['DOCUMENT_ROOT'].'/config/mimes.php'))
                {
                    $_mimes = require($_SERVER['DOCUMENT_ROOT'].'/config/mimes.php');
                }
                else
                {
                    $_mimes = array();
     ...
     * @return bool
     */
    public function resize($process)
    {
        
        if (!empty($process))
        {
            unset($process['create_thumb']); //i don't want thumb to be created by default
            $data                     = $this->data();
            ...
            if (!empty($process))
            {
                foreach ($process as $resizeOptions)
                {
                    if (!empty($resizeOptions['folder']))
                    {
                        $resizeOptions['new_image'] = $filePath . $resizeOptions['folder'] . '/' . $rawName . $extension;
                    }

                    $process = array_merge($config, $resizeOptions);
                    $process = new \Utility\ImageProcessor($process);
                    $process->resize();
                }
            }
            return true;
    ...
    public function crop($config)
    {
        $default       = [
            ...
            'quality'        => '100',
            'dynamic_output' => TRUE,
            'source_folder'  => \Config\Config::getUploadPath()
        ];
        $configuration = array_replace_recursive($default, $config);
        $process       = new \Utility\ImageProcessor($configuration);
        return $process->crop();
    }
    ...
    public function validate_upload_path()
    {
        ...
        if (!self::isReallyWritable($this->upload_path))
        {
    ...
    {
        $this->error_msg[] = !empty($this->messages[$msg]) ? $this->messages[$msg] : $msg;
        return $this;
    }
    ...
    public function multi_file_upload($file, $resizeConfig = [])
    {
        $files  = $_FILES;
