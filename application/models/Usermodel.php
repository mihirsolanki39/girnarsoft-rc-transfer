...
class Usermodel extends CI_Model
{
	...
	{		
		...
		$dataArr['is_admin']  = !empty($data['role_type']) ? $data['role_type'] : '1';
		...
		if ($data['id'] == '') {			
			...
			foreach ($roles as $role) {
				$rolling = explode('-', $role);
				$rolesArray = array(
					'user_id' => $id, 
					'team_id' => $rolling[1], 
					'role_id' => $rolling[0]
				);
				$results = $this->db->insert('crm_user_role_mapping', $rolesArray);
			}			
			$this->saveEmpBankLimit($data, $id);
			return $id;
		} else {
			// $roles = $dataArr['role_id'];
			// echo "<pre>";
			// print_r($roles);
			// foreach ($roles as $role) {
			// 	$idsss = explode('-', $role) ;
			// 	echo $idsss[0] .' - asdasd	'.$idsss[1];
			// }
			// exit;
			$dataArr['updated_by'] = '';
	...
	{
		$this->db->select('cr.id as role_id,cr.role_name,cr.team_id,cr.status,t.team_name');
		$this->db->from('crm_role as cr');
		// Explode string into array to make where_in clause work
		$this->db->join('crm_team_type as t', 't.id=cr.team_id', 'left');
    ...
    */
	...
	function getSupervisorRoleByTeamID($is_admin)
	{
		// $this->db->select('crm_user.*,cm.*');
		// $this->db->from("crm_user");
		// $this->db->join('crm_user_role_mapping as cm', 'cm.user_id=u.id', 'left');
		// $this->db->where('status', '1');
		...
		$this->db->select('u.id,u.name,u.email,u.role_id,u.team_id,u.user_code,u.status,u.is_admin,cm.*');
        $this->db->from('crm_user u');
        $this->db->join('crm_user_role_mapping cm', 'cm.user_id=u.id', 'left');
		$this->db->where('status', '1');        
        $this->db->where('u.is_admin', $is_admin);
