...
class Loginmodel extends CI_Model
{
    ...
    //     $this->db->select('u.id,u.name,u.email,u.password,u.mobile,u.dob,u.doj,u.dealer_id,u.user_code,u.status,u.is_admin,u.created_date,u.updated_date,t.team_name,r.role_name,d.is_classified,d.mobile as dealer_mobile,d.organization,d.address,d.default_showroom_id,d.email as dealer_email,cm.*');
    //     $this->db->from('crm_user u');
    //     //$query = $this->db->get_where('crm_user u', array('email' => $username,'password' => md5($password),'status'=>'1'));
    //     $this->db->join('crm_admin_dealers d', 'd.dealer_id=u.dealer_id and d.status="1"', 'left');
    //     $this->db->join('crm_user_role_mapping cm', 'cm.user_id=u.id', 'left');
    //     $this->db->join('crm_team_type t', 't.id=cm.team_id and t.status="1"', 'left');
    //     $this->db->join('crm_role r', 'r.id=cm.role_id and r.status="1"', 'left');
    ...
    public function getUserRole($userId){
        $this->db->select('cm.user_id,cm.team_id,cm.role_id');
        $this->db->from('crm_user_role_mapping as cm');
