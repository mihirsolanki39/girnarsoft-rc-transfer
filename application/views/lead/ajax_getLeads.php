...
<tr class="hover-section" id="tr_<?=$val['number']?>" >
        ...
        echo $new_date  = date('j M Y g:i a ', strtotime($enq_date)); } ?></span></div>
					<a data-toggle="modal" data-target="#update-details" onclick="getCustomerPopup(<?php echo $val['number']; ?>)">
            	...
            	<td class="font-14" style="position:relative">                    
                    <div  class="font-14 titleborder" id="req_edit"><strong>Requirements:</strong><span> <a href="javaScript:Void(0)"  id="editpreferences_<?=$val['number']?>"  class="flip font-12 pull-right text-link flip-down"><?php echo (isset($val['preferences']))?'EDIT':'ADD';?> </a></span></div>
...
<?php  } ?>
                    ...
                    <?php if($val['car_list']):?>
                    <div class="mrg-T5" >
                        <a  id="flip-down_<?=$val['number']?>" class="flip-down toggle text-link">					
    					 <span class="comment-v-more font-12">VIEW DETAILS</span> <!--<span class="comment-v-more font-12 mrg-L30">Recommend Cars</span>-->					
    					</a>
                    </div>
                    <?php endif;?>
                </td>
		    ...
		    <select data-toggle="modal" data-target="" name="status_<?=$val['leadID']?>" id="status_<?=$val['leadID']?>" class="form-control status_select" >
                    ...
                    <?php } ?>
            	    <div class="mrg-T10">
                    <select name="rating_<?=$val['leadID']?>" id="rating_<?=$val['leadID']?>" class="form-control" style="<?php if(strtolower($val['lead_status'])!='interested'){echo 'display:none';}?>">
            	...
            	<td>
                   ...
                   <div class="input-append date input-group" style="width:100%" id="followdate2" data-date="12-02-2012" data-date-format="dd-mm-yyyy">
                    ...
                    ?>
                    <input class="input-group buyer-followup-date span2 form-control add-on icon-cal1 dateTimeCalender pad-R30 font-13" size="12" type="text" id="followup_date_<?=$val['leadID']?>" name="followup_date_<?=$val['leadID']?>" value="<?php echo $followupDate; ?>"  placeholder="" readonly="readonly" style="cursor:pointer;">
				  ...
				  <div class="check-box-rm mrg-T15" id="Reminderdiv_<?=$val['leadID']?>"<?php if($val['lead_status_id'] != 4) { ?> style="display:none;"<?php } ?>>
				  	<label for="Reminder_<?=$val['leadID']?>"><span></span> Set Reminder</label>
				  </div>
<!-- 

                    <div class="check-box-rm mrg-T10">
                    <input type="checkbox" name="" id="Reminder" value="Reminder"><label for="Reminder" class="font-14 add-form"><span></span>Set Reminder</label>
                    </div>
 -->
				  <div class="input-append date input-group mrg-T5"  style="width:100%" id="followdate_<?=$val['leadID']?>" data-date="12-02-2012" data-date-format="dd-mm-yyyy" <?php if($val['lead_status_id'] != 4) { ?> style="display:none;"<?php } ?>>
                    <input class="span2 form-control width200 add-on icon-cal1 reminder-date font-13" size="16" type="text" id="reminder_date_<?=$val['leadID']?>" name="reminder_date_<?=$val['leadID']?>" value="<?php echo $walkinDate;?>"  placeholder="" readonly="readonly" style="cursor:pointer;">
                    <!--<span class="add-on input-group-addon"><span class="sprite icon-calander"></span></span>-->
                  </div>
                </td>
            	<td style="position:relative;width:180px;word-break:break-all;">               	
                            ...
                            <div class="font-14 ">
                                         ...
                                         // echo "<pre>";
                                          ...
                                          foreach ($val['history'] as $key=>$outerval){
                                             if(($val['lead_status_id']=='4') && (empty($outerval['status_change']['status_text'])))
                                              {
                                                     echo "<b>Status : </b>". $val['lead_status']."<br/>";
                                              }
                    ...
                    <textarea name="comment_<?=$val['leadID']?>" maxlength="200" id="comment_<?=$val['leadID']?>" placeholder="Add Comment" rows="1" class="form-control add-c-textBox comment is-maxlength mrg-T15" onkeyup="$(this).next().html((200 - this.value.length) + ' characters remaining (200 maximum)');" onclick="$(this).parent().siblings().find('.savecustomerdetail').css('background-color', $('#search').css('background-color'));
                        $(this).next().html((200 - this.value.length) + ' characters remaining (200 maximum)').show();"></textarea>
            	...
            	<td>
                    ...
                    <div class="width-save">
                                <span id="save_button">
                                    <button data-target="" data-toggle="modal" data-toggle="tooltip" title="SAVE" data-placement="top" class="btn text-btn btn-default font-12 btn-pad-sm updatelead" data-mobile="<?= $val['number'] ?>" data-leadid="<?= $val['leadID'] ?>" id="saveleadupdate_<?= $val['leadID'] ?>">SAVE</button>
                                            <!-- <a data-target="" data-toggle="modal" data-toggle="tooltip" title="Save" data-placement="top" title="Save" class="btn btn-default updatelead" data-mobile="<?= $val['number'] ?>" data-leadid="<?= $val['leadID'] ?>" id="saveleadupdate_<?= $val['leadID'] ?>">
                                            <i data-unicode="f0c7" class="fa fa-floppy-o font-18"></i> 
                                    </a> -->
                                </span>
                                <span >
                                    <a class="btn-shar" data-toggle="modal" data-target="#buyersmsEmail" data-placement="right" title="Share" class="buyersmsEmail" id="<?= $val['number'] ?>" onclick="sendSmsNewVersion(this.id, '', 'whatsup', '<?= $val['emailID'] ?>', 'refresh','<?= $val['leadID'] ?>')">
                                        <span class="pad-T10"><img src="<?= base_url()?>assets/admin_assets/images/shar-i.svg"></span>
                                    </a>  
                                    
        <!-- <a data-toggle="modal" data-target="#buyersms"  class="btn btn-default buyersms " href="javascript:void()" id="<?= $val['number'] ?>" onclick="sendSmsNewVersion(this.id, '', 'email', '<?= $customer_email ?>', 'refresh','<?= $lead_id ?>')" ><span data-toggle="tooltip" data-placement="top" title="Share" ><i class="fa fa-share-alt font-18" aria-hidden="true"></i></span></a> -->
                                    <span id="spnsavecustomerdetail_<?= $val['leadID'] ?>"></span>
                                </span>
