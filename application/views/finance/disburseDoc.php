...
?>
 <div class="sec-card">
                                       <ul class="nav nav-tabs nav-uptag" role="tablist">
                                          <li role="presentation" class="active"><a href="#upload" aria-controls="upload" role="tab" data-toggle="tab"  onclick="disbursement()" id="uploadimage">UPLOAD</a></li>
    ...
    // alert(case_id.'-'.customer_id);
      // Dropzone.autoDiscover = false;
            var myDropzone = new Dropzone("form#demoUpload", {
                url: '<?php echo base_url(); ?>Finance/uploadLoginDocs/'+case_id+"-"+customer_id+"-"+'2',
...
myDropzone.on("queuecomplete", function(file) {
  ...
  $('.loaderClas').attr('style','display:none;');
           ...
           var myDropzone1 = new Dropzone("div#secUploadBtn", {
                url: base_url+'Finance/uploadLoginDocs/'+case_id+"-"+customer_id+"-"+'2',
                previewsContainer: '#preview',
                success: function(file, response){
                  $('img[alt="'+file.name+'"]').parent().siblings('.tag').attr('id',$.trim(response));
                  //$('img[alt="'+file.name+'"]').parent().siblings('.tag').attr('onclick','tagImages(this)');
            ...
            //alert(customer_id+'-'+case_id);
             showImages(customer_id,case_id);
                $('.app-form').click(function(){
                    //$('.app-form.active').removeClass('active');
                    //$(this).parent('.child-drop').find('.app-form.active').removeClass('active');
                    $(this).addClass('active');
                    //$(".badg").removeClass('badge');
                   // $(this).children().addClass('badge');
                   if($(this).parents('.panel').find('.tick').attr('id') == "tick_12"){
                       var is_req = $(this).find('span').text();
                       if($.trim(is_req) == "*"){
                  ...
                  $('.loaderClas').attr('style','display:block;');  
                   //alert(customer_id);
                     $.ajax({
                      ...
                      data : {customer_id:customer_id,doctype:'disburse',case_id:case_id},
                      dataType: 'json',
        ...
        return true;
          ...
          function imageTagging(eve)
          {
            ...
            if((currImgId>0) && (tagdocId>0))
            {
              ...
              url : "<?php echo base_url(); ?>" + "Finance/loanTagMapping/",
              dataType: 'json',
              data : {ImgID:currImgId,taggID:tagdocId,customer_id:customer_id,type:'add',doctype:'disburse',case_id:case_id,subcat:subcatTagId[1]},
              success: function (response) { 
          ...
          $('#removeTag').click(function(){
            ...
            $.ajax({
              type : 'POST',
              url : "<?php echo base_url(); ?>" + "Finance/loanTagMapping/",
              data : {ImgID:image_id,customer_id:customer_id,type:'remove',doctype:'disburse',case_id:case_id},
              dataType: 'json',
              success: function (response) { 
                if(response.status)
                {        
         ...
         /* function updateCounter(tagId,type)
          {
              var counter= $('#subclass_'+tagId).text();
             // alert(counter);
              if(type=='add')
              {
                ...
                else
                {
                  ...
                  else if(countarr[1]>=1)
                  {
                    var  cin = countarr[1];
                  }
                  var abc = parseInt(cin)+parseInt(1);
                  $('#subclass_'+tagId).text(countarr[0]+' '+abc);               
                }
              }
              else
              {
                 if($.trim(counter)=='')
                  ...
                  else
                  {
                    ...
                    if(countarr[1]>=2)
                    {
                      var abc = parseInt(countarr[1])-parseInt(1);
                      $('#subclass_'+tagId).text(countarr[0]+' '+abc);
                    }
                    else
                    {
          ...
          function appBank(tag_id)
          {
              ...
              data : {type:'loan',customer_id:customer_id,tag_id:tag_id,doctype:'disburse',case_id:case_id},
              success: function (response) { 
          ...
          function updateBank()
          {
            ...
            $.ajax({
              type : 'POST',
              url : "<?php echo base_url(); ?>" + "Finance/loanTagMapping/",
              data : {bank:bank,taggID:tag_id,ImgID:img_id,customer_id:customer_id,type:'bank',doctype:'disburse',case_id:case_id},
              dataType: 'json',
              success: function (response) 
              { 
                //alert(response)
          ...
          function markIncorrect()
          {
            ...
            $.ajax({
              type : 'POST',
              url : "<?php echo base_url(); ?>" + "Finance/loanTagMapping/",
              data : {reason_id:reason,taggID:tag_id,ImgID:img_id,customer_id:customer_id,type:'markincorrect',doctype:'disburse',case_id:case_id},
              dataType: 'json',
              success: function (response) 
              { 
           ...
           $("#category_id").change(function() {
            ...
            $.ajax({
              ...
              data : {catId:catId,doctype:2,case_id:case_id},
              dataType: 'html',
              success: function (response) 
              { 
           ...
           function getCategoryList()
           {
            // var catId = $("#category_id").val();
              ...
              type : 'POST',
              url : "<?php echo base_url(); ?>" + "Finance/pendencyByCatId/",
              data : {type:'getcategoryId',doctype:2,case_id:case_id},
              dataType: 'html',
              success: function (response) 
              { 
...
function addPendency()
{
 ...
 // alert(category_id);
   $.ajax({
              ...
              data : {case_id:case_id,doctype:2,pendencyId:pendencyId,category_id:category_id},
              dataType: 'json',
              success: function (response) 
              { 
                if(response.status=='1'){
  ...
  else
  {
    return false;
        ...
        function savelogindoc()
        {
          ...
          var case_id = $("#case_id").val();
           $.ajax({
                  ...
                  data : {customer_id:customer_id,case_id:case_id,doctype:'2'},
                  dataType: 'json',
                  success: function (response) 
                  { 
         ...
         function imgtagName(tagid='',imag_id='')
        {
          if(imag_id=='')
          {
           ...
           $.ajax({
              ...
              data : {tagid:tagid,case_id:case_id,imag_id:imag_id,doctype:2},
              dataType : 'json',
        ...
        function downloadAll()
        {
          ...
          window.top.location.href = base_url+"Finance/getImagedownload/"+case_id+"/2 ";  
        }
      </script>
