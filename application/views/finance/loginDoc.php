<?php 
$styl = 'display:block';
if(((!empty($CustomerInfo['tag_flag'])) && ($CustomerInfo['tag_flag']=='4')) && ((!empty($rolemgmt[0]['role_name'])) && (($rolemgmt[0]['role_name']!='admin') || ($rolemgmt[0]['role_name']!='Loan Admin'))))
{
  $styl = 'display:none';
}
?>
<div class="sec-card">
   ...
   <div class="tab-content">
      ...
      <div role="tabpanel" class="tab-pane active" id="upload">
           ...
           <div class="col-md-12">
            <div class="downloadAllImg" style="text-align: right; position: absolute; right: 0;top: -140px;">
    ...
    <div class="col-md-12">
        <div class="btn-sec-width tag-btn-complete">
            <?php 
              ...
              $stylec = 'display:none';
              $action = '';
            if((($rolemgmt[0]['edit_permission']=='0') && (!empty($CustomerInfo['cust_bnk_id']))) || (($rolemgmt[0]['add_permission']=='0') && ($rolemgmt[0]['edit_permission']=='0')))
              {
                   ...
                   <div class="row">
                      <div class="col-md-12">
                         <div class="form-group">
                            <select class="form-control crm-form testselect1" name="bank_name" id="bank_name">
  ...
  $('.testselect1').SumoSelect({csvDispCount: 3, search: true, searchText:'Enter here.',triggerChangeCombined: true}); 
     ...
     var countloop = $('#countloop').val();
            var myDropzone = new Dropzone("form#demoUpload", {
                url: '<?php echo base_url(); ?>Finance/uploadLoginDocs/'+case_id+"-"+customer_id,
...
var myDropzone1 = new Dropzone("div#secUploadBtn", {
     url: base_url+'Finance/uploadLoginDocs/'+case_id+"-"+customer_id,
     previewsContainer: '#preview',
     success: function(file, response){
       $('img[alt="'+file.name+'"]').parent().siblings('.tag').attr('id',$.trim(response));
       //$('img[alt="'+file.name+'"]').parent().siblings('.tag').attr('onclick','tagImages(this)');
        ...
        $('.app-form').click(function(){
              ...
              if(cu!=''){
               ...
               // alert($(this).attr('rel'));
                ...
                var tag8 = 0;
                 if(dfdf[1] >6){
                      if($(this).parents('.panel').find('.tick').attr('id')  == "tick_8")
                      {
                              ...
                              if(tag8 == 1){
                                ...
                                $('li[rel='+currIDRel+']').parents('.panel').find('.tick').show(); $(this).parents('.panel').find('.error').hide();
                              }
                      }else{
                            $(this).parents('.panel').find('.tick').show();
                            $('li[rel='+currIDRel+']').parents('.panel').find('.tick').show(); $(this).parents('.panel').find('.error').hide();
...
function initial(imagesssss)
{
    ...
    var currImgsTg = $('#tagId').val();
      var currCl=$("#"+currImgsTg).attr('class');
...
$('#uploadimage').click(function(){
  ...
  function tickTag(currID,currCl){
      if(currID!='')
      {
       // alert('hii');
        //var ids = $(ev).attr('id');
    ...
    success: function (response) { 
       if(response.status=='1')
        ...
        else
        {
         ...
         var alreadyClass = $('.subr_'+subcatTagId[1]).attr('class');
         if(alreadyClass.indexOf('badge') != -1){
            //alert('badge' + " found");
          }else{
  ...
  $.ajax({
    ...
    success: function (response) { 
      ...
      { 
        ...
        $('#tagCount').html(countRemaining);
        var removetick = 0;
        if(response.parent_tag_id == 1 || response.parent_tag_id > 6){
...
//              }
        ...
        function downloadAll()
        {
          ...
          window.top.location.href = base_url+"Finance/getImagedownload/"+case_id;  
        }
