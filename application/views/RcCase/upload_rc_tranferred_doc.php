...
<div class="sec-card">
	<ul class="nav nav-tabs nav-uptag" role="tablist">
		<li role="presentation" class="active"><a href="#upload" aria-controls="upload" role="tab" data-toggle="tab" onclick="rctransferred()" id="uploadimage">UPLOAD</a></li>
		...
		<div role="tabpanel" class="tab-pane" id="tag">
									...
									<div class="panel panel-default">
										<div class="panel-heading" id="doc_<?= $key ?>">
											<h4 class="panel-title">
												<a data-toggle="collapse" data-parent="#accordion" href="#collapse<?= $i ?>" id="doct_<?= $key ?>" class="" aria-expanded="true">
													<?= $val['name'] ?> <?= $req ?>
			...
			<script>
				...
				var myDropzone = new Dropzone("form#demoUpload", {
					url: '<?php echo base_url(); ?>RcCase/uploadLoginDocs/' + case_id + "-" + customer_id + "-5",
				...
				var myDropzone1 = new Dropzone("div#secUploadBtn", {
					url: base_url + 'RcCase/uploadLoginDocs/' + case_id + "-" + customer_id + "-5",
					previewsContainer: '#preview',
					success: function(file, response) {
						$('img[alt="' + file.name + '"]').parent().siblings('.tag').attr('id', $.trim(response));
						//$('img[alt="'+file.name+'"]').parent().siblings('.tag').attr('onclick','tagImages(this)');
				...
				$(document).ready(function() {
					...
					$('.app-form').click(function() {
						...
						if (cu != '') {
							...
							var arr = ['121', '122'];
							if (dfdf[1] == '121') {
								var s260 = $('#subclass_121').text();
							}
							if (dfdf[1] == '122') {
								var s261 = $('#subclass_122').text();
							}
							if ((s260 >= 1) && (s261 >= 1)) {
				...
				function downloadAll() {
					...
					window.top.location.href = base_url + "RcCase/getImagedownload/" + case_id + "/5";
				}
			</script>