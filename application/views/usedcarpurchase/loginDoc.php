...
?>
                                             ...
                                             <form class="needsclick  <?php if(!empty($imageList)){?> dz-clickable bg-none <?}?>" <?php if(!empty($imageList)){?> style="border-width: 0px;" <?}?>   id="demoUpload">
                                                <div id="preview" <?php if(!empty($imageList)){?> class="ui-sortable"<?}?> >
                                                  ...
                                                  if(!empty($imageList)) {
                                                    ...
                                                    if(!empty($ival['tag_id']))
                                                    {
                                                      $tagId[$ival['tag_id']][]=$ival['tag_id'];
                                                  ...
                                                  <div class="dz-preview dz-image-preview">
                                                    <div class="dz-image">
                                                      <img data-dz-thumbnail="" alt="<?=$ival['sub_name']?>" src="<?=$ival['image_url']?>">
                                                    </div>
                                                    <div class="dz-details">
                                                      <div class="dz-size">
                                                      </div>
                                                      <div class="dz-filename">
                                                       <span data-dz-name=""><?=(!empty($ival['sub_name'])?$ival['sub_name']:'')?></span>
                                                      </div>
                                                    </div>
                                                    <div class="dz-progress">
                                                      <span class="dz-upload" data-dz-uploadprogress=""></span>
                                                    </div>
                                                    <div class="dz-error-message" <?php if(empty($ival['incorrect_image']) && ($ival['incorrect_image']==0)){ 
                                                      ?>style="display:none;" <?}?>>
                                                      <span data-dz-errormessage=""><?php if(!empty($ival['incorrect_image'])){ echo ($ival['incorrect_image']=='1')?'Incorrect Doc':'Unclear Image';}?></span>
                                                    </div>
                                                         ...
                                                         <!------>
                                                          <?php 
                                                           ...
                                                           // print_r($tagId);
                                                            ...
                                                            if(!empty($parent_id))
                                                            {
                                                              foreach ($parent_id as $ksel => $vsel) 
                                                              {
                                                                //echo  $vsel; exit;
                                                                //$style[$key]='';
                                                                if($key==$vsel)
      ...
      <input type="hidden" name="case_id" value="<?=(!empty($car_id))?$car_id:''?>" id="case_id"> 
   ...
   <!-- <div class="modal fade" id="addPendency" role="dialog">
     ...
     //alert(countloop+'adededesd');
      // Dropzone.autoDiscover = false;
            var myDropzone = new Dropzone("form#demoUpload", {
                url: '<?php echo base_url(); ?>UsedcarPurchase/uploadLoginDocs/'+case_id,
           ...
           var myDropzone1 = new Dropzone("div#secUploadBtn", {
                url: base_url+'UsedcarPurchase/uploadLoginDocs/'+case_id,
                previewsContainer: '#preview',
                uploadprogress:function(){
                   $('.searchresultloader').show();
                },
    ...
    {
      //alert(tagCountArr[i]);
      if(tagCountArr[i]=='null' || tagCountArr[i]=='0')
      {
  ...
  function delImg(eve)
  {
    ...
    $.ajax({
      ...
      success(){
          var no_on_images=$('#preview').children('.dz-image-preview').length;
          if(no_on_images==0){
              $('.downloadAllImg').hide();
          }
          $('.searchresultloader').hide();
      ...
      if(currID!='')
      {
        ...
        return true;
          ...
          function imageTagging(eve)
          {
            ...
            if((currImgId>0) && (tagdocId>0))
            {
              ...
              dataType: 'json',
              data : {ImgID:currImgId,taggID:tagdocId,type:'add',case_id:case_id,subcat:subcatTagId[1]},
       ...
       // alert('hi');
            ...
            $.ajax({
              type : 'POST',
              url : "<?php echo base_url(); ?>" + "UsedcarPurchase/loanTagMapping/",
              data : {ImgID:image_id,customer_id:customer_id,type:'remove',case_id:case_id},
              dataType: 'json',
              success: function (response) { 
                if(response.status)
                { 
         ...
         function updateCounter(tagId,type,classId)
          ...
          function appBank(tag_id,rel_id)
          {
              ...
              $.ajax({
              type : 'POST',
              url : "<?php echo base_url(); ?>" + "Inventories/getBankList/",
           ...
           function markIncorrect()
          {
            ...
            $.ajax({
              type : 'POST',
              url : "<?php echo base_url(); ?>" + "UsedcarPurchase/loanTagMapping/",
              data : {reason_id:reason,taggID:tag_id,ImgID:img_id,type:'markincorrect',case_id:case_id},
              dataType: 'json',
              success: function (response) 
              { 
           ...
           $("#category_id").change(function() {
            ...
            $.ajax({
              type : 'POST',
              url : "<?php echo base_url(); ?>" + "Inventories/pendencyByCatId/",
              data : {catId:catId,doctype:1,case_id:case_id},
              dataType: 'html',
              success: function (response) 
              { 
           ...
           function getCategoryList()
           {
              ...
              type : 'POST',
              url : "<?php echo base_url(); ?>" + "Inventories/pendencyByCatId/",
              data : {type:'getcategoryId',doctype:1,case_id:case_id},
              dataType: 'html',
              success: function (response) 
              { 
        ...
        function addPendency()
        {
          ...
          var category_id = $("#category_id").val();
           $.ajax({
                      type : 'POST',
                      url : "<?php echo base_url(); ?>" + "Inventories/addPendencyDoc/",
                      data : {case_id:case_id,doctype:1,pendencyId:pendencyId,category_id:category_id},
                      dataType: 'json',
                      success: function (response) 
                      { 
                        if(response.status=='1'){
        ...
        function downloadAll()
        {
          ...
          window.top.location.href = base_url+"UsedcarPurchase/getImagedownload/"+case_id;  
        }
        function savelogindoc()
        {
          ...
          var case_id = $("#case_id").val();
           $.ajax({
                  ...
                  data : {case_id:case_id,doctype:'1'},
                  dataType: 'json',
                  success: function (response) 
                  { 
        ...
        function imgtagName(tagid='',imag_id='')
        {
          ...
          if(imag_id=='')
          {
            ...
            $.ajax({
              ...
              data : {tagid:tagid,case_id:case_id,imag_id:imag_id,doctype:1},
              dataType : 'json',
