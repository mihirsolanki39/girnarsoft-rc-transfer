...
class User extends MY_Controller
{
    ...
    public function ajax_userList() {
        ...
        $params["results"] = $arr;
        $params["total_count"] = $arr_count;
        echo $datas = $this->load->view('employee/ajax_view_user', $params, true);
        exit;
    }
    ...
    public function getSuperVisor(){        
        $is_admin = $this->input->post('role_type');
        $supervisors = $this->Usermodel->getSupervisorRoleByTeamID($is_admin);

        $option= "<option value='' >Select Supervisor</option>";
        foreach ($supervisors as $super => $supervisor) {
            $option .="<option value='" . $supervisor['id'] . "' >" . $supervisor['name'] . "</option>";
        }
        echo $option;
