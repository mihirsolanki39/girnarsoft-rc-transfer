		...
		//alert(this);
			...
			//alert(imageUrl);
				$.ajax({
					...
					//var elm = jQuery(this);
					//var imageUrl = elm.attr('rel');
				//	alert(elm.parent('li').find('img').attr('src'));
				var d = new Date();
						elm.parent('li').find('img').attr('src',imageUrl+'?t='+d.getTime());
						//}
					}
				});
	})
...
/* done by lalit */	
		...
		var manageheight='130';
		var inc=0;
//var yinc;		
		 if(parseInt(theResponse)>12){
		 var quotient=Math.floor(parseInt(theResponse)/6);
		 var quotient1=Math.floor(parseInt(theResponse)%6);
		// alert(quotient);
		 //alert(quotient1);
		 if(quotient1>0)
		{
